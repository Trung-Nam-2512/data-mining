...
class MushroomInference:
    ...
    def load_model(self, model_path: str = None):
        """Load trained model"""
        # If model_path is None or empty, try to find best model
        if model_path is None or model_path == "":
            # Try to find model in models directory
            # Ưu tiên model mới nhất (epoch cao nhất) - thường là model tốt nhất
            model_files = list(MODELS_DIR.glob("best_model_*.pth"))
            if model_files:
                # Sort by epoch number (extract from filename)
                def get_epoch(filename):
                    try:
                        # Extract epoch number from "best_model_epoch_X.pth"
                ...
                model_files_sorted = sorted(model_files, key=get_epoch, reverse=True)
                model_path = str(model_files_sorted[0])
                print(f"Auto-selected model: {model_path} (epoch {get_epoch(model_path)})")
            else:
                raise FileNotFoundError(f"No model found in {MODELS_DIR}. Please train a model first.")
        
        # Now model_path should be a valid string
        model_path_obj = Path(model_path)
        if not model_path_obj.exists():
            raise FileNotFoundError(f"Model not found: {model_path}")
        
        # Use the Path object for the rest
        ...
        # Đảm bảo backbone được set đúng từ config
        backbone = config.get('backbone', 'unknown')
        if backbone == 'unknown':
            print("Warning: Backbone not found in config!")
        
        # Normalize backbone name (mobilenet_v3_large -> mobilenet_v3_large)
        ...
        # Load state dict với strict=False để tránh lỗi nếu có mismatch nhỏ
        try:
        ...
        # Verify backbone name is set correctly
        actual_backbone = self.model.backbone_name if hasattr(self.model, 'backbone_name') else backbone
        
        print(f"Model loaded successfully from {model_path_obj}")
        print(f"  Backbone: {actual_backbone}")
        print(f"  Number of classes: {num_classes}")
        print(f"  Classes: {', '.join(self.class_names)}")
        print(f"  Model file: {model_path_obj.name}")
